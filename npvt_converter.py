...
if __name__ == "__main__":
    ...
    try:
        ...
        if custom_remarks:
            # Append to remarks in the output file
            with open(args.output or "config.npv", 'r') as f:
                content = f.read()
            
            # Update remarks in the base64
            import base64, json
            try:
                lines = content.split('\n')
                b64_data = None
                for line in lines:
                    if 'BEGIN NPV' not in line and 'END NPV' not in line and not line.startswith('#'):
                        b64_data = line.strip()
                        break
                ...
                if b64_data:
                    ...
                    if args.npvt:
                        ...
                        new_content = "# NPV Config - Generated by g2ray\n"
                        new_content += "# Date: 2024\n"
                        new_content += "# Custom: " + custom_remarks + "\n"
                        new_content += "-----BEGIN NPV CONFIG-----\n"
                        new_content += encoded + "\n"
                        new_content += "-----END NPV CONFIG-----\n"
                    
                    with open(args.output or "config.npv", 'w') as f:
                        f.write(new_content)
                    print(f"✅ Added custom text: {custom_remarks}")
